...
export default function LeaderboardCard({ data, itemVariants }) {
    const rank = data?.userRank?.rank || "-";
    const xp = data?.userRank?.xp || 0;
    const nextRank = rank > 1 ? rank - 1 : 1;
    const xpNeeded = 50; // Placeholder logic, backend should ideally provide this
        ...
        <motion.div variants={itemVariants} className="bg-white rounded-3xl p-6 shadow-sm hover:shadow-md transition-all border border-gray-100 flex flex-col">
            ...
            <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                    {rank === 1 ? "You are at the top! 🥇" : (
                        <>Next goal: Rank #{nextRank} - <span className="text-green-600 font-bold">+{xpNeeded}XP</span></>
                    )}
                </p>
            </div>
